...
try {
  ...
  console.error(
    '[BackupAssistant] ❌ Missing dependencies! Please run "npm install" in the plugin directory. / 缺少依赖！请在插件目录下运行 "npm install"',
  );
}
...
function cleanTempFolder() {
  try {
    ...
    console.error(
      "[BackupAssistant] Failed to clean temp folder / 清理临时文件夹失败:",
      e,
    );
  }
}
...
function init(app, config) {
  ...
  app.post("/backup", async (req, res) => {
    if (currentTask.status === "working")
      return res.status(409).json({ error: "Task in progress / 任务进行中" });
    if (!archiver)
      return res.status(500).json({ error: "Missing dependencies / 缺少依赖" });
    ...
    archive.on("progress", (progress) => {
      const percent =
        totalBytes > 0 ? (progress.fs.processedBytes / totalBytes) * 100 : 50;
      updateStatus(
        percent,
        `Packing: ${Math.round(progress.fs.processedBytes / 1024 / 1024)}MB / 正在打包...`,
      );
    });
    ...
    if (extensions) {
      ...
      if (!data && fs.existsSync(P.extUser))
        archive.directory(P.extUser, "data/default-user/extensions");
    }
    if (themes) {
      archive.directory(P.themeGlobal, "public/themes");
      if (!data && fs.existsSync(P.themeUser))
        archive.directory(P.themeUser, "data/default-user/themes");
      if (fs.existsSync(P.movables))
        archive.directory(P.movables, "public/Movables");
    }
    ...
    if (secrets && fs.existsSync(path.join(rootDir, "secrets.json"))) {
      archive.file(path.join(rootDir, "secrets.json"), {
        name: "secrets.json",
      });
    }

    archive.append(
      JSON.stringify({ createdBy: "TavernBackupAssistant", version: "2.2" }),
      ...
      res.download(file, (err) => {
        if (!err) {
          try {
            fs.unlinkSync(file);
            console.log(
              `[BackupAssistant] Temp file cleaned: ${req.params.filename}`,
            );
          } catch (e) {}
        }
      });
  ...
  app.post("/restore", (req, res) => {
    ...
    writeStream.on("finish", async () => {
      try {
        ...
        updateStatus(
          100,
          "Success! Please refresh. / 还原成功！请刷新。",
          "done",
        );
...
module.exports = {
  ...
  info: {
    id: PLUGIN_ID,
    name: "Tavern Backup Assistant",
    description: "Easy backup and restore tools",
  },
};
